...
CliEnvValues loadFrom(String? Function(String key) lookup) {
  return CliEnvValues(
    baseUrl: _nonEmpty(lookup('EVGL_BASE_URL')) ?? 'http://127.0.0.1:8080',
    output: _nonEmpty(lookup('EVGL_OUTPUT')) ?? 'json',
    timeoutSeconds: _parseInt(lookup('EVGL_TIMEOUT_SECONDS'), 20),
    id: _nonEmpty(lookup('EVGL_ID')),
  );
}
...
Map<String, String> loadEnvMap(
  ...
  final out = <String, String>{};
  final baseUrl = pick(['EVGL_BASE_URL'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'http://127.0.0.1:8080');
  if (baseUrl != null) out['EVGL_BASE_URL'] = baseUrl;
  final output = pick(['EVGL_OUTPUT'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'json');
  if (output != null) out['EVGL_OUTPUT'] = output;
  final timeoutSeconds = pick(['EVGL_TIMEOUT_SECONDS'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, '20');
  if (timeoutSeconds != null) out['EVGL_TIMEOUT_SECONDS'] = timeoutSeconds;
  final id = pick(['EVGL_ID'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, null);
  if (id != null) out['EVGL_ID'] = id;
  return out;
}
