...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    base_url: nonEmpty(lookup("EVGL_BASE_URL")) ?? "http://127.0.0.1:8080",
    output: nonEmpty(lookup("EVGL_OUTPUT")) ?? "json",
    timeout_seconds: parseIntValue(lookup("EVGL_TIMEOUT_SECONDS"), 20),
    id: nonEmpty(lookup("EVGL_ID")),
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const base_url = pick(["EVGL_BASE_URL"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "http://127.0.0.1:8080");
  if (base_url !== undefined) out["EVGL_BASE_URL"] = base_url;
  const output = pick(["EVGL_OUTPUT"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "json");
  if (output !== undefined) out["EVGL_OUTPUT"] = output;
  const timeout_seconds = pick(["EVGL_TIMEOUT_SECONDS"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "20");
  if (timeout_seconds !== undefined) out["EVGL_TIMEOUT_SECONDS"] = timeout_seconds;
  const id = pick(["EVGL_ID"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (id !== undefined) out["EVGL_ID"] = id;
  return out;
}
