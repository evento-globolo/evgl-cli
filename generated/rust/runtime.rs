...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        base_url: lookup("EVGL_BASE_URL").filter(|value| !value.is_empty()).unwrap_or_else(|| "http://127.0.0.1:8080".to_string()),
        output: lookup("EVGL_OUTPUT").filter(|value| !value.is_empty()).unwrap_or_else(|| "json".to_string()),
        timeout_seconds: parse_int(lookup("EVGL_TIMEOUT_SECONDS"), 20),
        id: lookup("EVGL_ID").filter(|value| !value.is_empty()),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let base_url = pick(&["EVGL_BASE_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("http://127.0.0.1:8080"));
    if let Some(value) = base_url {
        out.insert("EVGL_BASE_URL".to_string(), value);
    }
    let output = pick(&["EVGL_OUTPUT"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("json"));
    if let Some(value) = output {
        out.insert("EVGL_OUTPUT".to_string(), value);
    }
    let timeout_seconds = pick(&["EVGL_TIMEOUT_SECONDS"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("20"));
    if let Some(value) = timeout_seconds {
        out.insert("EVGL_TIMEOUT_SECONDS".to_string(), value);
    }
    let id = pick(&["EVGL_ID"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = id {
        out.insert("EVGL_ID".to_string(), value);
    }
    Ok(out)
}
