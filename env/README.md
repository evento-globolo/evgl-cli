...
Secrets for `evento-globolo/evgl-cli` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** A PEM must be a single line with `\n` escapes:
  `JWT_PRIVATE_KEY_FILE=/run/secrets/jwt-signing-key.pem`
...
```sh
just env-docker-run dev ghcr.io/evento-globolo/evgl-cli:dev
